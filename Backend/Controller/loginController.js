...
exports.Login = (req, res) => {
  // JOIN employee table so we can fetch the department + position
  const sql = `
    SELECT l.*, e.department, e.position
    FROM tb_logins l
  ...
  db.query(sql, values, (err, data) => {
    ...
    if (data.length > 0) {
      ...
      bcrypt.compare(req.body.password, user.password, (bcryptErr, isMatch) => {
        ...
        if (isMatch) {
          // Save the correct user_id to session + department + position
          req.session.user = {
            user_id: user.user_id,
            email: user.email,
            role: user.role,
            // these will be null/undefined for non-employees and that’s fine
...
exports.CheckSession = (req, res) => {
  if (req.session.user) {
    // session.user now also contains department and position when available
