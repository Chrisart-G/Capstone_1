// src/components/User/RenewalBusinessPermitForm.jsx
...
export default function RenewalBusinessPermitForm() {
  ...
  const [formData, setFormData] = useState({
    applicationType: 'renewal',     // 🔒 fixed to renewal
  ...
  // =========================
  // 1) FETCH USER + BUSINESS INFO FOR AUTOFILL (same as NEW form)
  // =========================
  useEffect(() => {
    const fetchUserAndBusinessInfo = async () => {
      ...
      try {
        const [userRes, businessRes] = await Promise.all([
          ...
          axios
            .get(`${API_BASE_URL}/api/user-business-info`, {
              withCredentials: true,
            })
            .catch((err) => {
        ...
        if (userRes.data?.success) {
          ...
          setFormData((prevData) => ({
            ...
            // owner address from tbl_user_info.address
        ...
        if (businessRes.data?.success && businessRes.data.businessInfo) {
          ...
          setFormData((prevData) => ({
            ...
            businessTelephone: b.businessTelephone || prevData.businessTelephone,
            businessMobile: b.businessMobile || prevData.businessMobile,

            // Mirror business postal / tel to owner section if empty
            ownerPostalCode:
            ...
            ownerAddress: prevData.ownerAddress, // already from user info
            ownerEmail: prevData.ownerEmail,
            ownerMobile: prevData.ownerMobile,

            emergencyContact: b.emergencyContact || prevData.emergencyContact,
            emergencyPhone: prevData.emergencyPhone,
            ...
            femaleEmployees: b.femaleEmployees || prevData.femaleEmployees,
            localEmployees: b.localEmployees || prevData.localEmployees,
  ...
  // =========================
  // 1b) FETCH LATEST BUSINESS PERMIT FOR RENEWAL PREFILL
  // =========================
  useEffect(() => {
    const loadRenewalPrefill = async () => {
      try {
        const res = await axios.get(
          `${API_BASE_URL}/api/renewal-business-permit/prefill`,
          { withCredentials: true }
        );
        ...
        if (res.data && res.data.success && res.data.data) {
          ...
          setFormData((prev) => ({
            ...prev,
            ...prefill,
            applicationType: 'renewal', // ensure still renewal
          }));
        }
      } catch (err) {
        console.error('Failed to load renewal prefill data:', err);
      }
    };
  ...
  // =========================
  // 2) LOAD DRAFT FROM LOCALSTORAGE (separate key for renewal)
  // =========================
  useEffect(() => {
    try {
      const savedDraft = localStorage.getItem('renewalBusinessPermitDraft');
      if (savedDraft) {
        ...
        setDraftStatus('Loaded saved renewal draft.');
      }
    } catch (err) {
      console.error('Error loading renewal draft:', err);
    }
  }, []);
  ...
  const handleChange = (e) => {
    ...
    // applicationType is fixed to renewal in this form
  ...
  // shared validation function (same as new form)
  const validateForm = () => {
    ...
    // taxIncentiveEntity required only if yes
    if (
      formData.taxIncentive === 'yes' &&
      (!formData.taxIncentiveEntity ||
        formData.taxIncentiveEntity.toString().trim() === '')
    ) {
      newFieldErrors.taxIncentiveEntity = 'Please specify the entity.';
      if (!firstEmptyField) firstEmptyField = 'taxIncentiveEntity';
    }
  ...
  // actual submit logic (same style as new form)
  const submitForm = async () => {
    ...
    try {
      const payload = {
        ...formData,
        applicationType: 'renewal', // enforce
      };
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/BusinessPermit`,
        form,
        {
          withCredentials: true,
      ...
      if (response.data.success) {
        ...
        localStorage.removeItem('renewalBusinessPermitDraft');
        setDraftStatus(null);
        return true;
      } else {
  ...
  const handleSaveDraft = () => {
    try {
      localStorage.setItem(
        'renewalBusinessPermitDraft',
        JSON.stringify(formData)
      );
      setDraftStatus('Renewal draft saved locally on this device.');
    } catch (err) {
      console.error('Error saving renewal draft:', err);
      setDraftStatus(
  ...
  const handleFinalSubmit = async () => {
    const confirmed = window.confirm(
      'Are you sure you want to submit this renewal application? You will not be able to edit it after submitting.'
    );
    if (!confirmed) return;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
            ...
            <p className="text-gray-700 mb-4">
              Your business permit renewal application has been submitted
              successfully.
            </p>
  ...
  return (
    <div>
        ...
        {isLoadingUserInfo && (
          <div className="flex justify-center items-center py-4 mb-4">
            ...
            <span className="ml-2 text-gray-600">
              Loading user & business information...
            </span>
          </div>
        )}
        ...
        <form ref={formRef} onSubmit={handleSubmit}>
          ...
          <section className="mb-8">
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-4">
                {/* TYPE OF APPLICATION: fixed RENEWAL */}
                <div className="flex items-center space-x-4">
                  <span className="font-medium mr-2">TYPE OF APPLICATION:</span>
                  <span className="px-3 py-1 rounded bg-gray-100 font-semibold">
                    RENEWAL
                  </span>
                </div>
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="amendmentFrom" className="block mb-1">
                      FROM:
                    </label>
                    <select
                      id="amendmentFrom"
          ...
          <section className="mb-8">
            <div className="flex items-center justify-between mb-4">
              ...
              {/* Always renewal on this form, so note always visible */}
      ...
      {/* Review Modal – same style as new form, but for renewal */}
      {isReviewOpen && (
