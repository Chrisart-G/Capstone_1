...
/* CORS note:
   Ensure your backend allows credentials and JSON size ~35mb for signature images. */
...
export default function FencingInlineForm({ applicationId, onClose, onSubmitted }) {
  const API =
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const r = await fetch(`${API_BASE}/user/fencing/form?${q.toString()}`, { credentials: "include" });
  ...
  const buildPayload = (opts = { includeSigs: false }) => {
    const keep = (o) => {
      const x = { ...o };
      if (!opts.includeSigs) delete x.signature_data_url;
      return x;
    };
    const payload = {
      box2: keep(box2),
  ...
  async function saveDraft() {
    setSaving(true);
    try {
      const r = await fetch(`${API_BASE}/user/fencing/form/save`, {
        method: "POST",
        credentials: "include",
  ...
  async function stampSignature(which, dataUrlFromState) {
    try {
      ...
      const r = await fetch(`${API_BASE}/user/fencing/form/sign`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  async function doSubmit() {
    setSubmitting(true);
    try {
      const r = await fetch(`${API_BASE}/user/fencing/form/submit`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  if (loading) {
    return (
      <InlineModal open title="Fencing Permit – Fill Boxes 2–5" onClose={onClose}>
  ...
  return (
    <InlineModal open title="Fencing Permit – Fill Boxes 2–5" onClose={onClose}>
              ...
              <section className="space-y-3">
                ...
                <div className="flex flex-wrap gap-4 text-sm">
                  <label className="inline-flex items-center gap-2">
                    <input type="radio" name="role_fenc" checked={box3.role === "pe"} onChange={() => setBox3((v) => ({ ...v, role: "pe" }))} />
                    Professional Electronics Engineer
                  </label>
                  <label className="inline-flex items-center gap-2">
                    <input type="radio" name="role_fenc" checked={box3.role === "re"} onChange={() => setBox3((v) => ({ ...v, role: "re" }))} />
                    Registered Electronics Engineer
                  </label>
                  <label className="inline-flex items-center gap-2">
                    <input type="radio" name="role_fenc" checked={box3.role === "rme"} onChange={() => setBox3((v) => ({ ...v, role: "rme" }))} />
      ...
      <ConfirmModal
        ...
        message={
          "We will render your current entries (Boxes 2–5) onto the official FENCING template.\n" +
