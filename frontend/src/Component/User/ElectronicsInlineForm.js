// src/components/User/ElectronicsInlineForm.jsx
import React, { useEffect, useRef, useState } from "react";

/* Small tip:
   If your server runs on a different origin, make sure CORS allows credentials,
   and the server has express.json({ limit: '10mb' }) or higher. */
...
export default function ElectronicsInlineForm({ applicationId, onClose, onSubmitted }) {
  const API =
  ...
  const [pane, setPane] = useState("preview"); // mobile: 'preview' | 'form'

  // Boxes
  const [box2, setBox2] = useState({
    engineer_name: "", date: "", address: "", prc_no: "", validity: "",
    ptr_no: "", date_issued: "", issued_at: "", tin: "", signature_data_url: "",
  });
  const [box3, setBox3] = useState({
    role: "pe", signed_name: "", date: "", prc_no: "", validity: "",
    ptr_no: "", date_issued: "", issued_at: "", tin: "", address: "",
  });
  const [box4, setBox4] = useState({ owner_name: "", date: "", address: "", signature_data_url: "" });
  const [box5, setBox5] = useState({ lot_owner_name: "", date: "", address: "", signature_data_url: "" });

  // temp signature for box3 (draw/stamp preview)
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const r = await fetch(`${API_BASE}/user/electronics/form?${q.toString()}`, {
          credentials: "include",
        });
  ...
  const buildPayload = (opts = { includeSigs: false }) => {
    ...
    // ADD: include Box 3 signature on submit (preview uses temp state)
  ...
  async function saveDraft() {
    setSaving(true);
    try {
      const r = await fetch(`${API_BASE}/user/electronics/form/save`, {
        method: "POST",
        credentials: "include",
  ...
  async function stampSignature(which, dataUrlFromState) {
    try {
      ...
      const r = await fetch(`${API_BASE}/user/electronics/form/sign`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  async function doSubmit() {
    setSubmitting(true);
    try {
      const r = await fetch(`${API_BASE}/user/electronics/form/submit`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  if (loading) {
    return (
      <InlineModal open title="Electronics Permit – Fill Boxes 2–5" onClose={onClose}>
    ...
    <InlineModal open title="Electronics Permit – Fill Boxes 2–5" onClose={onClose}>
        ...
        <div className="grid md:grid-cols-2 gap-4 px-3 md:px-4 pb-4">
          ...
          <div className={`${pane === "form" ? "hidden md:block" : ""}`}>
            ...
            <div className="text-xs text-gray-500 mt-2">
              Stamp signatures to see placement. On Submit, Box 2/3/4/5 signatures and printed fields are embedded by the server.
            </div>
          </div>
              ...
              <section className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <label className="text-sm">
                    ...
                    <input className="mt-1 w-full border rounded px-2 py-1"
                      value={box2.engineer_name} onChange={(e) => setBox2((v) => ({ ...v, engineer_name: e.target.value }))} />
                  </label>
                  <label className="text-sm">
                    <span className="block text-gray-600">Date</span>
                    <input type="date" className="mt-1 w-full border rounded px-2 py-1"
                      value={box2.date} onChange={(e) => setBox2((v) => ({ ...v, date: e.target.value }))} />
                  </label>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <label className="text-sm">
                    ...
                    <input className="mt-1 w-full border rounded px-2 py-1"
                      value={box2.address} onChange={(e) => setBox2((v) => ({ ...v, address: e.target.value }))} />
                  </label>
                  <div />
                  <label className="text-sm">
                    <span className="block text-gray-600">PRC No.</span>
                    <input className="mt-1 w-full border rounded px-2 py-1"
                      value={box2.prc_no} onChange={(e) => setBox2((v) => ({ ...v, prc_no: e.target.value }))} />
                  </label>
                  <label className="text-sm">
                    <span className="block text-gray-600">Validity</span>
                    <input className="mt-1 w-full border rounded px-2 py-1"
                      value={box2.validity} onChange={(e) => setBox2((v) => ({ ...v, validity: e.target.value }))} />
                  </label>
                  <label className="text-sm">
                    <span className="block text-gray-600">PTR No.</span>
                    <input className="mt-1 w-full border rounded px-2 py-1"
                      value={box2.ptr_no} onChange={(e) => setBox2((v) => ({ ...v, ptr_no: e.target.value }))} />
                  </label>
                  <label className="text-sm">
                    <span className="block text-gray-600">Date Issued</span>
                    <input type="date" className="mt-1 w-full border rounded px-2 py-1"
                      value={box2.date_issued} onChange={(e) => setBox2((v) => ({ ...v, date_issued: e.target.value }))} />
                  </label>
                  <label className="text-sm">
                    <span className="block text-gray-600">Issued at</span>
                    <input className="mt-1 w-full border rounded px-2 py-1"
                      value={box2.issued_at} onChange={(e) => setBox2((v) => ({ ...v, issued_at: e.target.value }))} />
                  </label>
                  <label className="text-sm">
                    <span className="block text-gray-600">TIN</span>
                    <input className="mt-1 w-full border rounded px-2 py-1"
                      value={box2.tin} onChange={(e) => setBox2((v) => ({ ...v, tin: e.target.value }))} />
                  </label>
                </div>
              ...
              <section className="space-y-3">
                ...
                <div className="flex flex-wrap gap-4 text-sm">
                  <label className="inline-flex items-center gap-2">
                    <input type="radio" name="role_elec" checked={box3.role === "pe"} onChange={() => setBox3((v) => ({ ...v, role: "pe" }))} />
                    Professional Electronics Engineer
                  </label>
                  <label className="inline-flex items-center gap-2">
                    <input type="radio" name="role_elec" checked={box3.role === "re"} onChange={() => setBox3((v) => ({ ...v, role: "re" }))} />
                    Registered Electronics Engineer
                  </label>
                  <label className="inline-flex items-center gap-2">
                    <input type="radio" name="role_elec" checked={box3.role === "rme"} onChange={() => setBox3((v) => ({ ...v, role: "rme" }))} />
                    Registered Master Electrician (if applicable)
                  </label>
                </div>
              ...
              <section className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <label className="text-sm">
                    ...
                    <input className="mt-1 w-full border rounded px-2 py-1"
                      value={box4.owner_name} onChange={(e) => setBox4((v) => ({ ...v, owner_name: e.target.value }))} />
                  </label>
                  <label className="text-sm">
                    <span className="block text-gray-600">Date</span>
                    <input type="date" className="mt-1 w-full border rounded px-2 py-1"
                ...
                <SignatureField title="Owner Signature (Box 4)"
              ...
              <section className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <label className="text-sm">
                    ...
                    <input className="mt-1 w-full border rounded px-2 py-1"
                      value={box5.lot_owner_name} onChange={(e) => setBox5((v) => ({ ...v, lot_owner_name: e.target.value }))} />
                  </label>
                  <label className="text-sm">
                    <span className="block text-gray-600">Date</span>
                    <input type="date" className="mt-1 w-full border rounded px-2 py-1"
                ...
                <SignatureField title="Lot Owner Signature (Box 5)"
      ...
      <ConfirmModal
        ...
        message={
          ...
          "All signatures (Box 2/3/4/5) and printed fields are embedded by the server.\n\n" +
          "Proceed?"
        }
